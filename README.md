...
> **Warning**  
> ループバックデバイスに対して IPv6 が有効な環境において `localhost` を用いて接続するとクライアント実装によって IPv6 アドレスである
> `[::1]` への接続を優先してタイムアウトを待つために応答まで時間がかかるケースがみられます，
> これは当エンジンがデフォルトで `127.0.0.1` に対してのみバインドするため `[::1]` での接続を受けられないことに由来します．
> こうした事象に遭遇した場合，当エンジンを起動する際に `--host ::1` オプションを使うことで `[::1]` に対してバインドするように設定でき，
> 応答速度の改善が期待できます．
