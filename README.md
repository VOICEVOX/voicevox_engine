...
VOICEVOX API での音声合成は、エンジン側で音声合成パラメータ`AudioQuery`の初期値を作成してユーザーに返し、ユーザーが必要に応じてパラメータを編集したあと、エンジンがパラメータに従って音声合成することで実現しています。
パラメータ作成は`/audio_query`エンドポイントで、音声合成は`/synthesis`エンドポイントで行っており、最低この２つに対応すれば VOICEVOX API に準拠したことになります。
