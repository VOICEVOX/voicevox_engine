...
class UserDictionaryManager:
    ...
    def _save_dict(self, user_dict: dict[str, UserDictWord]) -> None:
        """ユーザー辞書をファイルへ保存する。"""
        # 辞書データを保存向け形式へ変換し、JSONファイルへ書き込む
    ...
    @_mutex_wrapper(mutex_user_dict)
    def _load_dict(self) -> dict[str, UserDictWord]:
        """ユーザー辞書をファイルから読み出す。"""
        # 辞書データをJSONファイルから読み出し、利用向け形式へ変換する
        # NOTE: 指定ユーザー辞書が存在しない場合、空辞書を返す
    ...
    def import_user_dict(
        ...
        # 重複エントリの上書き
        if override:
            new_user_dict = {**old_user_dict, **dict_data}
        # 重複エントリの保持
        else:
            new_user_dict = {**dict_data, **old_user_dict}

        self._save_dict(new_user_dict)
        self._apply_dict()
