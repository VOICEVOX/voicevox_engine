"""/mora_pitch API のテスト。"""

import pytest
from fastapi.testclient import TestClient
from syrupy.assertion import SnapshotAssertion

from test.e2e.single_api.utils import gen_mora
from test.utility import (
    FRAME_DECODE_STYLE_ID,
    NOT_EXIST_STYLE_ID,
    SING_STYLE_ID,
    round_floats,
)

_accent_phrases = [
    {
        "moras": [
            gen_mora("テ", "t", 2.3, "e", 0.8, 3.3),
            gen_mora("ス", "s", 2.1, "U", 0.3, 0.0),
            gen_mora("ト", "t", 2.3, "o", 1.8, 4.1),
        ],
        "accent": 1,
        "pause_mora": None,
        "is_interrogative": False,
    }
]


def test_post_mora_pitch_200(
    client: TestClient, snapshot_json: SnapshotAssertion
) -> None:
    response = client.post("/mora_pitch", params={"speaker": 0}, json=_accent_phrases)
    assert response.status_code == 200
    assert snapshot_json == round_floats(response.json(), 2)


@pytest.mark.parametrize(
    "style_id", [NOT_EXIST_STYLE_ID, FRAME_DECODE_STYLE_ID, SING_STYLE_ID]
)
def test_post_mora_pitch_with_invalid_style_422(
    client: TestClient, style_id: int, snapshot_json: SnapshotAssertion
) -> None:
    response = client.post(
        "/mora_pitch", params={"speaker": style_id}, json=_accent_phrases
    )
    assert response.status_code == 422
    assert snapshot_json == response.json()
