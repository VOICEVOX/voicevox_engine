...
def test_mocked_create_accent_phrases_output(
    ...
    # Outputs
    result = tts_engine.create_accent_phrases(hello_hiho, StyleId(1), False)
    # Tests
    assert snapshot_json == round_floats(pydantic_to_native_type(result), round_value=2)
...
def _create_synthesis_test_base(text: str) -> list[AccentPhrase]:
    ...
    return tts_engine.create_accent_phrases(text, StyleId(1), False)
...
def test_create_accent_phrases() -> None:
    ...
    actual = tts_engine.create_accent_phrases(text, StyleId(1), False)
